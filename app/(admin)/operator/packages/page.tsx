  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-semibold">Packages</h1></div>
        <button type="button" onClick={openCreate} className="rounded bg-slate-800 px-3 py-1.5 text-sm text-white hover:bg-slate-700">+ New package</button>
      </div>
      ...
      {form && (
        <div className="mt-4 rounded border border-slate-200 bg-white p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="flex flex-col gap-1 text-sm">
              <span className="font-medium text-slate-600">duration (days)</span>
      ...
      <div className="mt-4 overflow-x-auto rounded border border-slate-200 bg-white">
        <table className="w-full min-w-[860px] text-left text-sm">
          ...
          <tbody>
            {rows.length === 0 && <tr><td colSpan={8} className="px-3 py-6 text-center text-slate-400">No packages</td></tr>}
            {rows.map((r) => (
