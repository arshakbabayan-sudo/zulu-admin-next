...
export default function OperatorVisasPage() {
  ...
  function openCreate() { setEditId(null); setForm({ country: "", visa_type: "", price: undefined, currency: "USD", processing_days: undefined }); setFormErr(null); }
  function openEdit(r: VisaRow) { setEditId(r.id); setForm({ country: r.country ?? "", visa_type: r.visa_type ?? "", price: r.price ?? undefined, currency: r.currency ?? "USD", processing_days: r.processing_days ?? undefined }); setFormErr(null); }
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-semibold">Visas</h1><p className="mt-1 text-sm text-zinc-500">GET /api/visas · POST · PATCH · DELETE</p></div>
        <button type="button" onClick={openCreate} className="rounded bg-zinc-900 px-3 py-1.5 text-sm text-white hover:bg-zinc-700">+ New visa</button>
      </div>
      ...
      {form && (
        <div className="mt-4 rounded border border-zinc-200 bg-white p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {(["country", "visa_type", "currency"] as const).map((f) => (
            ...
            {(["price", "processing_days"] as const).map((f) => (
              <label key={f} className="flex flex-col gap-1 text-sm">
                <span className="font-medium text-zinc-600">{f.replace(/_/g, " ")}</span>
      ...
      <div className="mt-4 overflow-x-auto rounded border border-zinc-200 bg-white">
        <table className="w-full min-w-[700px] text-left text-sm">
          ...
          <tbody>
            {rows.length === 0 && <tr><td colSpan={6} className="px-3 py-6 text-center text-zinc-400">No visas</td></tr>}
            {rows.map((r) => (
