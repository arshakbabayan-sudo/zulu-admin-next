      ...
      <Table>
        ...
        <TBody>
          ...
          {rows.map((r) => {
            ...
            const title = r.transfer_title ?? "?";
            const initials = getInitials(title);
            const route = [r.pickup_city, r.dropoff_city].filter(Boolean).join(" → ");
            return (
            <TR key={r.id}>
              ...
              <TD>
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="font-medium text-fg-t8 truncate">{r.transfer_title ?? "—"}</div>
                    {route ? (
