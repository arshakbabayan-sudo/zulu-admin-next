  ...
  return (
    <div>
      <div className="mb-4 text-sm">
        <Link href="/support/tickets" className="text-fg-t6 underline">
          ← Tickets
        </Link>
      </div>
        ...
        <div className="mt-3 text-sm">
          <label className="text-fg-t6">
            ...
            <input
              ...
              className="ml-2 w-28 rounded border border-default px-2 py-1 tabular-nums"
          ...
          <button
            type="button"
            onClick={() => load()}
            className="ml-2 rounded border border-default bg-white px-2 py-1"
          >
            Reload
          </button>
        </div>
      ...
      {ticket && (
        <div className="mt-4 space-y-4">
          <div className="rounded border border-default bg-white p-4 text-sm">
            <div className="font-medium">{ticket.subject}</div>
            <div className="mt-2 text-fg-t6">
              Status: {ticket.status} | Priority: {ticket.priority} | Company:{" "}
              {ticket.company_id ?? "-"}
            </div>
            <div className="mt-1 text-xs text-fg-t7">
              User: {ticket.user ? `${ticket.user.name} <${ticket.user.email}>` : "-"}
            </div>
          </div>
          <div className="rounded border border-default bg-white p-4">
            <h2 className="text-sm font-semibold text-fg-t11">Thread</h2>
              ...
              {ticket.messages.map((m) => (
                <li
                  ...
                  className={
                    ...
                    (m.is_admin_reply ? "border-amber-200 bg-amber-50" : "border-default bg-figma-bg-1")
                  }
                >
                  <div className="text-xs text-fg-t7">
                    {m.is_admin_reply ? "Admin" : "User"}
                    {m.user ? ` | ${m.user.name}` : ""} | {m.created_at ?? ""}
                  </div>
                  <div className="mt-1 whitespace-pre-wrap text-fg-t11">{m.message}</div>
                </li>
              ))}
            </ul>
          </div>
          <div className="rounded border border-default bg-white p-4">
            <h2 className="text-sm font-semibold text-fg-t11">Admin reply</h2>
            <textarea
              value={reply}
              onChange={(e) => setReply(e.target.value)}
              rows={4}
              className="mt-2 w-full max-w-xl rounded border border-default px-2 py-1 text-sm"
            ...
            <button
              ...
              onClick={() => sendReply()}
              className="mt-2 rounded border border-default bg-white px-3 py-1 text-sm disabled:opacity-40"
            >
              Send reply
            </button>
          </div>
