...
export default function PlatformBookingsPage() {
  ...
  async function handleConfirm(id: number) {
    ...
    setBusyId(id);
    try { await apiConfirmBooking(token, id); await load(); }
  ...
  async function handleCancel(id: number) {
    ...
    setBusyId(id);
    try { await apiCancelBooking(token, id); await load(); }
    catch (e) { alert(e instanceof ApiRequestError ? e.message : "Failed"); }
    finally { setBusyId(null); }
  }

  if (!allowed || forbidden) return (
    <div><h1 className="text-xl font-semibold">Bookings</h1><div className="mt-4"><ForbiddenNotice /></div></div>
  );

  return (
    <div>
      <h1 className="text-xl font-semibold">Bookings</h1>
      <p className="mt-1 text-sm text-zinc-500">GET /api/bookings · confirm · cancel</p>
      ...
      <div className="mt-4 overflow-x-auto rounded border border-zinc-200 bg-white">
        <table className="w-full min-w-[900px] text-left text-sm">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={9} className="px-3 py-6 text-center text-zinc-400">No bookings found</td></tr>
            )}
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-zinc-100 hover:bg-zinc-50">
                ...
                <td className="px-3 py-2 tabular-nums">
                  {r.total_amount != null ? `${r.currency ?? ""} ${Number(r.total_amount).toFixed(2)}` : "—"}
                </td>
